...
checkboxChange(e){
  ...
  newArray.forEach(e=>{
    list.forEach(e2=>{
      if(e==e2.id){
        money = money+e2.goodsNum*e2.price*e2.goods.field1/10;
      }
    })
  })
  //重新设置totalPrice
  ...
  toOrder(e){
    ...
    wx.request({
      ...
      data: { 
        ...
        list:this.data.cartIdList.join(',') 
      },
      success: res => {
        if (res.data.code == '0000') {
          app.countCartNum();//计算购物车数量
          wx.showToast({
            title: '请求成功！',
            duration: 2000,
            mask: true,
...
calculateCartIdList(e){
  ...
  minusNum(e){
    ...
    console.log(e);
    let id=e.currentTarget.dataset.id;
    let cartObj=e.currentTarget.dataset.obj;
    if(cartObj.goodsNum==1){
      Toast.show('fail', '最少购买一个！', 2000);
      return false;
    }
    wx.request({
      url: rootUrl + '/wx/cart/updateNum',
      data: {'id': id,'number':-1},
      success: res => {
        if (res.data.code == '0000') {
          wx.showToast({
